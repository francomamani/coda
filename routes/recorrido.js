...
router.post('/adicionar', function (req, res, next) {
    lista.push(new Fraternidad(req.body.id));
    res.send(lista);
});
...
router.get('/reset', function (req, res, next) {
    ...
    for (var i = 0; i<lista.length; i++) {
        lista[i].setHoraInicio(null);
        lista[i].setHoraActual(null);
        lista[i].setHoraFinal(null);
        lista[i].setNombreUbicacion("");
        lista[i].setEstado(0);
        lista[i].setDistancia(0);
    }
    control = 0;
    res.send(lista);
});
